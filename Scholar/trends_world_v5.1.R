...
interest_data=data.frame(interest_data)%>%
  select(- inter_fec)%>%
  ...
  select(keyword,gprop,agno,hits,date)%>%
  distinct()
...
country_data=gtrend_full%>%
        select(keyword,gprop,agno,hits,location)%>%
  mutate(hits=replace_na(hits,0))

country_data=country_data%>%
  group_by(location,keyword,gprop,agno)%>%
  mutate(hits=sum(hits))%>%
  distinct()%>%
  group_by(keyword,gprop,agno)%>%
  mutate(max_hits=max(hits))%>%
  mutate(min_hits=0)%>%
  mutate(rescal=trunc((hits-min_hits)/(max_hits-min_hits)*100))%>%
  mutate(hits = as.integer(rescal),
